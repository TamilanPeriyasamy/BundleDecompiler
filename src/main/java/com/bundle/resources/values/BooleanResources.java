...
public class BooleanResources  extends Resources {
	...
	public BooleanResources() throws ParserConfigurationException {
	...
	/**
	 */
	public void parseBooleanResources(File currentXmlFile) throws Exception {
		...
		while((currentLine =bufferedReader.readLine()) != null){
			...
			if (currentLine.trim().startsWith(Resources.mParseResourceEntry +ApplicationResources.mApplicationId)) {
			...
			}else if (currentLine.trim().startsWith("(") && ( currentLine.contains(") true") || currentLine.contains(") false"))) {
				//() (attr) type=reference src=/home/lakeba13/workspace
				String dumpValues[] = currentLine.trim().split(" ");
				attributeResType = dumpValues[1].trim();
				...
				System.err.println(BooleanResources.class.getCanonicalName()+" "+currentLine);
			}
		}
